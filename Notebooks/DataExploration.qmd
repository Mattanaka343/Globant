...
Si bien se cuenta con la descripción general de los datos de todos los empleados también resulta importante evaluar el nivel de engagement medio por equipo para hacer comparativas entre la eficiencia de todos los equipos
### División por equipos de trabajo
```{python}
team_describe = pd.DataFrame()
data_by_team= []
teams = list(data['team_name'].unique())
for team in teams:
    data_by_team.append(data[data['team_name'] == team])
    team_describe[team]=data[data['team_name'] == team]["engagement"].describe()

print(team_describe)

```

Los valores obtenidos a partir de describe resultan útiles pero sería mas conveniente visualizarlos de forma más clara en gráficas de barras o boxplots.

```{python}
means = team_describe.loc["mean"]
stds = team_describe.loc["std"]

# Obtenemos los index de los valores ordenados de la media
order = means.sort_values(ascending=True).index

#utilizamos este index para reordenar nuestras metricas
...
ax.set_xlabel('Value')
ax.set_title('Engagement medio por equipo')

# Etiquetas ordenadas
ax.set_yticks(x + width, teams_sorted)

ax.legend(loc='lower right')
ax.set_xlim(0, 4)

plt.savefig('../Figures/BarplotEquipos.png')
plt.show()


```

Ahora tenemos una visualización clara del orden de los equipos en base a la media del engagement y a su vez de sus desviaciones estandar que en la mayoría de los casos parece muy similar.

En base a estas metricas el equipo con mejor engagement es "Star Wars: The Furry Awakens" y el peor siendo "Breaking Badger"



```{python}

# Extraer valores de engagement y nombres de equipo
engagement_values = []
team_labels = []

for df in data_by_team:
    engagement_values.append(df["engagement"])
    team_labels.append(df["team_name"].iloc[0])  # tomar el nombre del equipo


fig=plt.boxplot(engagement_values, labels=team_labels, vert=False)

plt.xlabel("Engagement")
plt.title("Distribución de Engagement por Equipo")
plt.savefig('../Figures/BoxplotEquipos.png')
plt.show()


```

Pasando al boxplot podemos observar una cantidad significativa de outliers en el rango más bajo del engagement, esto muestra que existe un porcentaje considerable de miembros en cada equipo cuya participación e interacción suele ser escasa. Cabe destacar que la media de todos los equipos fluctúa en valores cercanos a 3, ya sea ligeramente menor o mayor que este. A su vez, los equipos Fight Club Penguin y The Matrix Mice resaltan por poseer muchos outliers con niveles altos de engagement.

### Division por meses

```{python}
months = [1,2,3,4,5,6]
month_describe = pd.DataFrame()
data_by_month= []
for month in months:
    data_by_month.append(data[data['date'].dt.month == month])

    month_describe[month]=data[data['date'].dt.month == month]["engagement"].describe()
#
print(month_describe)
```


```{python}

means = month_describe.loc["mean"]
stds = month_describe.loc["std"]
...
ax.set_xlabel('Value')
ax.set_title('Engagement medio por mes')

# Etiquetas ordenadas
ax.set_yticks(x + width, month_names)

ax.legend(loc='upper right')
ax.set_xlim(0, 4)

plt.savefig('../Figures/BarplotMes.png')
plt.show()

```
A partir de esta gráfica es fácil distinguir al mes de enero como el mes más productivo de los registros.
...
for df in data_by_month:
    engagement_values.append(df["engagement"])
 # tomar el nombre del equipo


plt.boxplot(engagement_values, labels=month_names, vert=False)

plt.xlabel("Engagement")
plt.title("Distribución de Engagement por Mes")
plt.savefig('../Figures/BoxplotMes.png')
plt.show()


```

Este gráfico refuerza los análisis previamente mencionados, el primer mes del año se alza como el mes con el mejor promedio de engagement, e igualmente, se observan bastantes outliers en el intervalo de 0-1, siendo un factor común en todos los meses, aunque ligeramente menor en enero.
...
fig, axs = plt.subplots(int(np.ceil(n/2)),2, figsize= (22,22));
...
plt.savefig('../Figures/EngagementMes.png')
plt.tight_layout()
plt.show()
```
